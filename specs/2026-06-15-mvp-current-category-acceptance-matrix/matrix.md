...
| `MOVILIDAD/BICICLETAS Y PATINETAS` | `¿Cuál es el deducible del seguro de bicicletas y patinetas?` | `¿Qué cubre el seguro para bicicletas y patinetas?` | `pv bicis y patinetas v2.pdf`; `clausulado-bicis y patinetas.pdf` | P2 | `fragile-pass` | 2026-06-15 after `bicicletas-patinetas-coverage-policy-family-recovery`, the explicit coverage smoke query stayed inside `clausulado-bicis y patinetas.pdf`, while the deductible smoke query again ranked `pv bicis y patinetas v2.pdf` first after narrowing the coverage-only routing rule. Hosted manual Space QA on 2026-06-20 then reopened the deductible path because the public UI drifted outside the PV family without local chunk-artifact support; `hosted-manual-bicis-deductible-and-choque-procedure-retrieval-recovery` now restores a hosted-safe guide-family routing rule and requires revalidation. |
...
| `MOVILIDAD/TRANSVERSALES` / `choque simple` | `¿Cómo debo tomar fotos en un choque simple?` | `¿Cuál es el procedimiento de atención del choque simple?` | `como tomar fotos choque simple v2.pdf`; `proceso atencion choque simple v2.pdf`; `circular choque simple.pdf` | P3 | `fragile-pass` | 2026-06-15 after `choque-simple-intent-evidence-routing-recovery`, photo intent ranked `como tomar fotos choque simple v2.pdf` first, while procedure intent was anchored on `proceso atencion choque simple v2.pdf` with circular support. Hosted manual Space QA on 2026-06-20 then reopened the procedure path because the public UI drifted back toward photo-only evidence; `hosted-manual-bicis-deductible-and-choque-procedure-retrieval-recovery` now restores a hosted-safe procedure-family routing rule and requires revalidation. |
