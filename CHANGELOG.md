...
- Add the `suscripción` mobility transversal onboarding and corrective retrieval sequence by onboarding `politicas de suscripcion de movilidad.pdf`, normalizing its semantic section structure, diversifying broad policy retrieval away from heading stubs and repeated `14.1` chunks, preserving normalized `14.6.1`/`14.6.2` lineage, aligning collective-billing intent ahead of individual financing content, and closing the final `14.6.2` lead-selection gap so live retrieval now returns the cleaner billing explanation chunk first.
